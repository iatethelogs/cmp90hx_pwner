...
launch_tui() {
...

    {
        ...
        printf '\n'
        printf 'rc=$?\n'
        printf 'printf "\\n\\nPROCESS EXITED WITH CODE %%s\\nLOG: %%s\\nPress Enter to close this tmux session..." "$rc" %q\n' "$LOG"
        printf 'read -r _ </dev/tty || true\n'
        printf 'exit "$rc"\n'
    } > "$runner"
    ...
    tmux new-session -d -s "$session" -n "$PROGRAM_NAME" "bash '$runner'"
...
cmdlog() { printf '\n[%s] %s\n' "$(date -Is)" "$*"; }
ok() { ui '%b[ OK ]%b %s\n' "${GREEN}${BOLD}" "$RST" "$*"; }
warn() { ui '%b[WARN]%b %s\n' "${YELLOW}${BOLD}" "$RST" "$*"; cmdlog "WARN: $*"; }
fail() { ui '%b[FAIL]%b %s\n' "${RED}${BOLD}" "$RST" "$*"; cmdlog "FAIL: $*"; }
...
sanitize_depmod() { find /etc/depmod.d -type f \( -name '*cmp90hx*' -o -name '*rejoin*' -o -name '*pwner*' \) -print -delete 2>/dev/null || true; }

remove_gen2_autostart_artifacts() {
    systemctl disable --now "$SERVICE_NAME" 2>/dev/null || true
    rm -f "/etc/systemd/system/$SERVICE_NAME" "/lib/systemd/system/$SERVICE_NAME" "/usr/lib/systemd/system/$SERVICE_NAME" 2>/dev/null || true
    rm -f /etc/profile.d/cmp90hx-pwner-login.sh /etc/profile.d/cmp90hx-pwner-firstboot.sh /etc/update-motd.d/99-cmp90hx-pwner 2>/dev/null || true
...
EOF_GRUB_NOUVEAU
    ...
    if lsmod | grep -q '^nouveau'; then
        ...
        if [[ "$AUTO_REBOOT_IF_NOUVEAU" == "1" && "$count" == "0" ]]; then
            echo 1 > "$count_file"; sync; systemctl reboot; sleep 60
        fi
...
purge_old_pwner() {
    ...
    rm -f /etc/systemd/system/cmp90hx-pwner-firstboot-ui.service /etc/systemd/system/cmp90hx-pwner-ssh-gate.service 2>/dev/null || true
...
nvidia_uninstall_best_effort() {
    ...
    if command -v nvidia-uninstall >/dev/null 2>&1; then nvidia-uninstall --silent --no-runlevel-check 2>/dev/null || true; fi
...
download_with_retry() {
    local url="$1" out="$2" tmp attempts delay i
    attempts="${DOWNLOAD_ATTEMPTS:-10}"; delay="${DOWNLOAD_RETRY_DELAY:-8}"; tmp="${out}.part"
    mkdir -p "$(dirname "$out")"
    for ((i=1; i<=attempts; i++)); do
        printf 'download attempt %s/%s: %s\n' "$i" "$attempts" "$url"
        rm -f "$tmp"
        if command -v curl >/dev/null 2>&1; then
            curl -fL --connect-timeout 30 --retry 2 --retry-delay 5 --retry-all-errors -o "$tmp" "$url" && { mv -f "$tmp" "$out"; return 0; }
        elif command -v wget >/dev/null 2>&1; then
            wget --tries=3 --waitretry=5 --timeout=30 --read-timeout=60 -O "$tmp" "$url" && { mv -f "$tmp" "$out"; return 0; }
        else
...
git_clone_retry() {
    local repo="$1" dst="$2" attempts delay i
    attempts="${GIT_ATTEMPTS:-6}"; delay="${GIT_RETRY_DELAY:-8}"
    rm -rf "$dst"
    for ((i=1; i<=attempts; i++)); do
        printf 'git clone attempt %s/%s: %s\n' "$i" "$attempts" "$repo"
        git clone --depth 1 "$repo" "$dst" && return 0
        rm -rf "$dst"; printf 'git clone failed, retry in %ss\n' "$delay"; sleep "$delay"
    done
    return 1
}
...
install_stock_driver() {
    ...
    [[ -s "$NVIDIA_RUN" ]] || download_with_retry "$NVIDIA_RUN_URL" "$NVIDIA_RUN" || return 1
    chmod +x "$NVIDIA_RUN"
    bash "$NVIDIA_RUN" --silent --accept-license --no-questions --no-cc-version-check --no-nouveau-check || return 1
    depmod -a || true
}

filter_upstream_install_log() {
    sed -u \
        -e '/installing systemd unit/d' \
        -e '/cmp90hx-gen2\.service/d' \
        -e '/apply now (or reboot)/d' \
        -e '/current_link_speed/d' \
        -e '/pcie\.link\.gen\.current/d'
}

install_patched_compute_unlock_driver() {
    mkdir -p "$(dirname "$PROJECT_DIR")"
    git_clone_retry "$PROJECT_REPO" "$PROJECT_DIR" || return 1
    cd "$PROJECT_DIR" || return 1

    printf 'running upstream patched driver installer, Gen2 autostart output filtered and removed after install\n'
    bash scripts/install.sh 2>&1 | filter_upstream_install_log
    local rc=${PIPESTATUS[0]}
    remove_gen2_autostart_artifacts
    return "$rc"
}

copy_manual_gen2_runtime_from_project() {
    mkdir -p "$PREFIX"
    local f src copied=0
    for f in rejoin16-cycle.sh cmp90hx-gen2-handoff.sh cmp90hx-gen2-minimal.sh maskread.py bar0poke; do
        for src in "$PROJECT_DIR/scripts/$f" "$PROJECT_DIR/$f" "$PREFIX/$f"; do
            if [[ -e "$src" ]]; then
                cp -a "$src" "$PREFIX/$f" 2>/dev/null || true
                chmod +x "$PREFIX/$f" 2>/dev/null || true
    ...
    preserve_rejoin_verifiers || true
    remove_gen2_autostart_artifacts
    (( copied >= 4 ))
}
...
preserve_rejoin_verifiers() {
    ...
    if [[ -n "$bin" && -x "$bin" && "$bin" != "$PREFIX/cmpunlocker-rs" ]]; then
        install -m 0755 "$bin" "$PREFIX/cmpunlocker-rs" && printf 'saved verifier: %s -> %s\n' "$bin" "$PREFIX/cmpunlocker-rs"
        copied=1
    fi
    check="$(find_rejoin_check_sh || true)"
    if [[ -n "$check" && -f "$check" && "$check" != "$PREFIX/check.sh" ]]; then
        install -m 0755 "$check" "$PREFIX/check.sh" && printf 'saved check.sh: %s -> %s\n' "$check" "$PREFIX/check.sh"
        copied=1
    fi
    [[ "$copied" == "1" ]] || true
}
...
bind_cmps_to_nvidia() {
    local cmps cmp dev drv okc=0 total=0
    ...
    (( ${#cmps[@]} > 0 )) || { printf 'no CMP 90HX 10de:220d devices found\n'; return 32; }
    for cmp in "${cmps[@]}"; do
        total=$((total + 1)); dev="/sys/bus/pci/devices/$cmp"; drv="none"
        [[ -L "$dev/driver" ]] && drv="$(basename "$(readlink -f "$dev/driver")")"
        if [[ "$drv" != "nvidia" ]]; then
            printf '%s: binding to nvidia driver\n' "$cmp"
            if [[ "$drv" != "none" && -e "$dev/driver/unbind" ]]; then printf '%s' "$cmp" > "$dev/driver/unbind" 2>/dev/null || true; sleep 1; fi
        ...
        printf '%s driver=%s\n' "$cmp" "$drv"
        [[ "$drv" == "nvidia" ]] && okc=$((okc + 1))
    done
    printf 'nvidia-bound CMP cards: %s/%s\n' "$okc" "$total"
    [[ "$okc" == "$total" ]]
}

verify_compute_driver_layer() {
    local ver src
...
card_is_gen2(){ local c="$1" u s e up; u="$(upstream_of "$c")"; s="$(cat "/sys/bus/pci/devices/$c/current_link_speed" 2>/dev/null || true)"; e="$(lspci -Dvv -s "$c" | grep 'LnkSta:' | head -1 || true)"; up="$(lspci -Dvv -s "$u" | grep 'LnkSta:' | head -1 || true)"; [[ "$s" == *"5.0 GT/s"* && "$e" == *"Speed 5GT/s"* && "$up" == *"Speed 5GT/s"* ]]; }
verify_links(){ local c u s w e up total=0 ok=0; for c in $(find_cmps); do total=$((total+1)); u="$(upstream_of "$c")"; s="$(cat "/sys/bus/pci/devices/$c/current_link_speed" 2>/dev/null || true)"; w="$(cat "/sys/bus/pci/devices/$c/current_link_width" 2>/dev/null || true)"; e="$(lspci -Dvv -s "$c" | grep 'LnkSta:' | head -1 || true)"; up="$(lspci -Dvv -s "$u" | grep 'LnkSta:' | head -1 || true)"; log "$c upstream=$u speed=$s width=$w"; log "  endpoint $e"; log "  upstream $up"; card_is_gen2 "$c" && ok=$((ok+1)); done; log "GEN2 $ok/$total"; [[ "$total" != 0 && "$ok" == "$total" ]]; }
failed_cards(){ local c; for c in $(find_cmps); do card_is_gen2 "$c" || printf '%s\n' "$c"; done; }
failed_feat_cards(){ local c f; for c in $(failed_cards); do f="$(mask_val "$c" "$MASK_FEAT")"; [[ "$f" != "0xffffffff" ]] && printf '%s\n' "$c"; done; }
need_runtime(){ [[ -x "$PREFIX/cmp90hx-gen2-handoff.sh" ]] || { log 'missing handoff script'; exit 11; }; [[ -x "$PREFIX/cmp90hx-gen2-minimal.sh" ]] || { log 'missing minimal script'; exit 12; }; [[ -x "$PREFIX/rejoin16-cycle.sh" ]] || { log 'missing rejoin16-cycle script'; exit 13; }; [[ -r "$PREFIX/maskread.py" ]] || { log 'missing maskread.py'; exit 14; }; [[ -x "$PREFIX/bar0poke" ]] || { log 'missing bar0poke'; exit 15; }; grep -q 'via resource0' "$PREFIX/rejoin16-cycle.sh" 2>/dev/null && { log 'FAIL: bad direct-writer shim detected'; exit 16; }; }
handoff(){ log 'handoff'; bash "$PREFIX/cmp90hx-gen2-handoff.sh"; }
run_minimal(){ local label="$1" reset="$2" tries="$3"; log "WAIT: $label can take several minutes. Do not stop it."; log "Gen2 runtime: $label pci_reset=$reset tries=$tries"; CMP90HX_PCI_RESET="$reset" CMP90HX_RESET_ON_STUCK=1 CMP90HX_OPEN_REHANDOFF=1 CMP90HX_STUCK_REPEAT_LIMIT=8 CMP90HX_MASK_OPEN_TRIES="$tries" bash "$PREFIX/cmp90hx-gen2-minimal.sh"; sleep 5; }
soft_pass(){ log "=== adaptive soft pass: handoff -> minimal, tries=$SOFT_MASK_TRIES ==="; handoff; run_minimal 'adaptive soft pass' 0 "$SOFT_MASK_TRIES" || true; verify_links; }
aggressive_pass(){ log "=== aggressive pass: handoff -> minimal with PCI reset, tries=$AGGR_MASK_TRIES ==="; rm -rf /run/cmp90hx-gen2-reset-done 2>/dev/null || true; handoff; run_minimal 'aggressive pass' 1 "$AGGR_MASK_TRIES" || true; verify_links; }
...
soft_pass && { log 'Gen2 verified after hard FEAT fallback + final soft pass'; exit 0; }
...
apply_gen2_now() { write_apply_script || return 1; bash "$APPLY_SCRIPT"; }

install_cuda_toolkit() {
    export DEBIAN_FRONTEND=noninteractive
    local os_id os_ver distro arch keyring_deb keyring_url
    os_id=""; os_ver=""; [[ -r /etc/os-release ]] && { . /etc/os-release; os_id="${ID:-}"; os_ver="${VERSION_ID:-}"; }
    arch="$(dpkg --print-architecture 2>/dev/null || true)"
    [[ "$arch" == "amd64" ]] || { printf 'unsupported architecture: %s\n' "$arch"; return 2; }
    case "${os_id}:${os_ver}" in ubuntu:20.04) distro="ubuntu2004" ;; ubuntu:22.04) distro="ubuntu2204" ;; ubuntu:24.04) distro="ubuntu2404" ;; debian:11) distro="debian11" ;; debian:12) distro="debian12" ;; *) printf 'unsupported distro: %s %s\n' "$os_id" "$os_ver"; return 2 ;; esac
    apt-get update && apt-get install -y ca-certificates curl wget gnupg lsb-release || return 1
    ...
    download_with_retry "$keyring_url" "$keyring_deb" || return 1
    dpkg -i "$keyring_deb" || return 1
    apt-get update && apt-get install -y cuda-toolkit || return 1
...
KIOCSOUND = 0x4B2F
DIVISOR = int(1193180 / 1000)
for dev in ("/dev/console", "/dev/tty0"):
    try:
        fd = os.open(dev, os.O_WRONLY)
        for _ in range(4):
            fcntl.ioctl(fd, KIOCSOUND, DIVISOR); time.sleep(0.15)
            fcntl.ioctl(fd, KIOCSOUND, 0); time.sleep(0.15)
        os.close(fd); break
    except Exception:
        pass
PY_BEEP4
...
EOF_BEEP_UNIT
    systemctl daemon-reload && systemctl enable "$BOOT_BEEP_SERVICE"
...
#!/usr/bin/env bash
set -u
command -v nvidia-settings >/dev/null 2>&1 || { echo "nvidia-settings is missing"; exit 1; }
export DISPLAY="${DISPLAY:-:0}"
for g in $(nvidia-smi --query-gpu=index --format=csv,noheader 2>/dev/null); do nvidia-settings -a "[gpu:${g}]/GPUFanControlState=1" || true; nvidia-settings -a "[fan:${g}]/GPUTargetFanSpeed=100" || true; done
EOF_FAN100
...
command -v nvidia-settings >/dev/null 2>&1 || { echo "nvidia-settings is missing"; exit 1; }
export DISPLAY="${DISPLAY:-:0}"
for g in $(nvidia-smi --query-gpu=index --format=csv,noheader 2>/dev/null); do nvidia-settings -a "[gpu:${g}]/GPUFanControlState=1" || true; nvidia-settings -a "[fan:${g}]/GPUTargetFanSpeed=60" || true; done
EOF_FAN60
...
command -v nvidia-settings >/dev/null 2>&1 || { echo "nvidia-settings is missing"; exit 1; }
export DISPLAY="${DISPLAY:-:0}"
for g in $(nvidia-smi --query-gpu=index --format=csv,noheader 2>/dev/null); do nvidia-settings -a "[gpu:${g}]/GPUFanControlState=0" || true; done
EOF_FANAUTO
    cat > "$HELPER_BIN_DIR/gpu-full" <<'EOF_GPUFULL'
#!/usr/bin/env bash
set -u
command -v nvidia-smi >/dev/null 2>&1 || { echo "nvidia-smi is missing"; exit 1; }
for g in $(nvidia-smi --query-gpu=index --format=csv,noheader 2>/dev/null); do nvidia-smi -i "$g" -pm 1 || true; max_pl=$(nvidia-smi -i "$g" --query-gpu=power.max_limit --format=csv,noheader,nounits 2>/dev/null | awk '{print int($1)}'); [[ "$max_pl" =~ ^[0-9]+$ && "$max_pl" -gt 0 ]] && nvidia-smi -i "$g" -pl "$max_pl" || true; nvidia-smi -i "$g" -rgc || true; done
EOF_GPUFULL
...
for g in $(nvidia-smi --query-gpu=index --format=csv,noheader 2>/dev/null); do nvidia-smi -i "$g" -pm 1 || true; nvidia-smi -i "$g" -pl "$IDLE_POWER_LIMIT" || true; nvidia-smi -i "$g" -lgc "$MIN_CLOCK,$MAX_CLOCK" || true; done
EOF_GPUIDLE
...
install_compute_unlock() {
    ...
    clear_left; banner
    note 'Compute unlock only. No PCIe Gen2 apply. No Gen2 autostart service.'
    note 'Gen2 is manual and volatile; use menu item 2 after reboot.'
    run_step 'backup current state' backup_state || return $?
    run_step 'stop GPU users' stop_gpu_users || return $?
    run_step 'remove previous pwner install' purge_old_pwner || return $?
    run_step 'remove previous NVIDIA driver' nvidia_uninstall_best_effort || return $?
    run_step 'install dependencies' apt_install_base || return $?
    run_step 'block nouveau' blacklist_nouveau || return $?
    run_step 'install stock NVIDIA driver' install_stock_driver || return $?
    run_step 'install patched compute unlock driver' install_patched_compute_unlock_driver || return $?
    run_step 'copy manual Gen2 runtime files' copy_manual_gen2_runtime_from_project || return $?
    run_step 'verify compute unlock' verify_rejoin_compute_full || return $?
    ok 'COMPUTE UNLOCK INSTALLED'
    warn 'Gen2 was not applied. Reboot if needed, then run menu item 2.'
    pause_return
}
...
show_apply_gen2() {
    ...
    clear_left; banner
    note 'Applying PCIe Gen2 now. Exact sequence: soft 13 -> aggressive 13 -> hard target reset fallback -> final soft.'
    note 'This can take 5-60 minutes. Do not close it and do not start GPU load.'
    run_step 'apply PCIe Gen2 now; wait, this can be long' apply_gen2_now && ok 'PCIe GEN2 APPLY COMPLETE' || warn 'Gen2 did not converge. Reboot and try menu item 2 again.'
    pause_return
}

show_verify_compute() { STEP_NO=0; TOTAL_STEPS=2; clear_left; banner; run_step 'compute driver layer' verify_compute_driver_layer || true; run_step 'rejoin compute full' verify_rejoin_compute_full && ok 'COMPUTE VERIFY COMPLETE' || warn 'COMPUTE VERIFY FAILED; see right log pane.'; pause_return; }
show_verify_gen2() { STEP_NO=0; TOTAL_STEPS=1; clear_left; banner; run_step 'PCIe Gen2 link' verify_links && ok 'GEN2 VERIFY COMPLETE' || warn 'GEN2 VERIFY FAILED; apply Gen2 or reboot and retry.'; pause_return; }
show_install_cuda() { STEP_NO=0; TOTAL_STEPS=1; clear_left; banner; run_step 'install CUDA toolkit' install_cuda_toolkit && ok 'CUDA TOOLKIT INSTALLED'; pause_return; }
show_install_boot_beep() { STEP_NO=0; TOTAL_STEPS=1; clear_left; banner; run_step 'install 4-beep at start' install_boot_beep && ok '4-BEEP INSTALLED'; pause_return; }
show_install_helpers() { STEP_NO=0; TOTAL_STEPS=1; clear_left; banner; run_step 'install fan/gpu helpers' install_fan_gpu_helpers && ok 'FAN/GPU HELPERS INSTALLED'; pause_return; }
...
uninstall_all() {
    ...
    local answer=''; [[ -t 0 ]] && read -r answer
...
Notes:
  Compute unlock is installed separately.
...
main() {
    case "${1:-}" in
        ...
        --unlock-this-shit|--unlock) warn 'Combined unlock-all mode was removed. Running compute install only.'; install_compute_unlock ;;
        --help|-h) usage ;;
        '')
            while true; do
                menu
                IFS= read -r choice
                case "$choice" in
                    1) run_menu_action install_compute_unlock ;;
                    2) run_menu_action show_apply_gen2 ;;
                    3) run_menu_action show_verify_compute ;;
                    4) run_menu_action show_verify_gen2 ;;
                    5) run_menu_action show_install_cuda ;;
                    6) run_menu_action show_install_boot_beep ;;
                    7) run_menu_action show_install_helpers ;;
                    8) ui 'Type UNINSTALL to remove everything: '; IFS= read -r confirm; [[ "$confirm" == "UNINSTALL" ]] && run_menu_action uninstall_all || warn 'cancelled' ;;
                    0) exit 0 ;;
